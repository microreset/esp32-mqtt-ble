        ...
        if (strcmp(advertisedDevice.getAddress().toString().c_str(), BLETrackedDevices[i].address.c_str()) == 0) {
          ...
          if (!BLETrackedDevices[i].isDiscovered) {
            ...
            if (advertisedDevice.getRSSI() > -80) {
              ...
              DEBUG_PRINT(F("INFO: Device position: In the "));
            ...
            // If out the proximity zone:
              ...
              DEBUG_PRINT(F("INFO: Device position: Outside the "));
              DEBUG_PRINTLN(F(LOCATION));
            }
          }
        } else {
...
void setup() {
  ...
  for (uint8_t i = 0; i < NB_OF_BLE_TRACKED_DEVICES; i++) {
      ...
      // MQTT binary sensor: /presence/<LOCATION>/<BLE_ADDRESS>
      // /sensor/bedroom/e092e56930ef/state AWAY
      sprintf(mqttTopic, MQTT_SENSOR_TOPIC_TEMPLATE, LOCATION, tmp_ble_address);
...
void loop() {
  ...
  if (enableWifi) {
    ...
    for (uint8_t i = 0; i < NB_OF_BLE_TRACKED_DEVICES; i++) {
      if (BLETrackedDevices[i].toNotify) {
        if (BLETrackedDevices[i].isDiscovered && BLETrackedDevices[i].isInZone) {
            publishToMQTT(BLETrackedDevices[i].mqttTopic, MQTT_PAYLOAD_ON);
        } else if (BLETrackedDevices[i].isDiscovered && !BLETrackedDevices[i].isInZone) {
